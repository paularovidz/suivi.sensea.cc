<template>
  <div class="text-center py-8">
    ...
    <template v-else-if="booking">
      ...
      <!-- Clothing tip -->
      <div class="mt-8 p-4 bg-amber-50 border border-amber-200 rounded-lg max-w-md mx-auto text-left">
        <div class="flex">
          ...
          <div class="ml-3">
            <p class="text-sm text-amber-800">
              <strong>Conseil :</strong> Pensez à vous habiller confortablement, idéalement une tenue de sport ou des vêtements souples.
            </p>
          </div>
        </div>
      </div>
      ...
      <!-- Reminder -->
      <div class="mt-4 p-4 bg-indigo-50 rounded-lg max-w-md mx-auto text-left">
        <div class="flex">
...
onMounted(async () => {
  ...
  try {
    ...
    error.value = toastStore.parseApiError(err) || 'Impossible de confirmer la réservation'
  } finally {
    loading.value = false
  }
})
